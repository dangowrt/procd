...
int main(int argc, char **argv)
{
	...
	/*
	 * phase 1: consume top-level options before the verb.
	 * scanned by hand so the verb's getopt_long below starts fresh.
	 */
	for (i = 1; i < argc; ++i) {
		const char *a = argv[i];
	...
	/* phase 2: dispatch on the verb; each verb parses its own argv */
	optind = 1;
	opterr = 1;
